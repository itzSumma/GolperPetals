...
export default function Footer(): React.JSX.Element {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-4">
            <Link href="/" className="text-3xl font-extrabold text-red-600 tracking-tighter  ">
              Golper<span className="text-rose-700 hover:text-black/45  text-3xl">Petals</span>
            </Link>
  ...
  Even when life keeps you busy, your love deserves to be felt. Create a bouquet that carries your heart, and let every bloom say what words cannot.
            ...
            <div className="pt-4 border-t border-red-100">
              <p className="text-md font-extrabold text-red-900">Owner</p>
              <p className="text-lg text-rose-600 font-semibold italic">Sharmin Sultana Summa</p>
            </div>
          </div>
              ...
              <ul className="space-y-4">
                {section.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-gray-700 hover:text-red-600 transition-colors duration-400 font-medium "
                    >
                      {link.label}
                    </Link>
                  </li>
